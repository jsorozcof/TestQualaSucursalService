...
To update the database use this command from the Web project folder (replace `Quala.SucursalService` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Quala.SucursalService.Infrastructure/Quala.SucursalService.Infrastructure.csproj -s Quala.SucursalService.Web.csproj
```
